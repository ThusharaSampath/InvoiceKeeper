...
public class ContentFetcher {
	...
	public static void updateTable(JTable ui_table, String db_table) {
		...
		String sql = "SELECT InvoiceNo,ChequeNo,InvoiceAmount,DealerId,DealerName,InvoiceDate,InvoiceDeadline,Short FROM "
...
//TODO add dealer name to invoice and cheque tables
	...
	public static void updateTable_by_dealer(JTable ui_table, String db_table, String Dealername) {
		...
		Statement stmt = null;
//TODO add dealer name to invoice and cheque tables
		String sql = "SELECT `InvoiceNo`, `ChequeNo`, `InvoiceAmount`, `DealerName`, `InvoiceDate`, `InvoiceDeadline`,'Short' FROM `invoice` WHERE `DealerName` LIKE '"+Dealername+"%'";
...
//TODO add dealer name to invoice and cheque tables
	...
	public static void updateTable_cheque(JTable ui_table, String db_table) {
		...
		Statement stmt = null;
		// TODO add dealer name to invoice and cheque tables
		String sql = "SELECT `ChequeNo`, `InvoiceNo`, `DealerName`, `ChequeAmount`, `ChequeDate`, `ChequeExpireDate`, `ChequeExpireStatus`, `ChequePassStatus` FROM `cheque` ORDER BY `ChequeDate`";
		try {
			stmt = connection.prepareStatement(sql);
			ResultSet rs = stmt.executeQuery(sql);
			ui_table.setModel(DbUtils.resultSetToTableModel(rs));
	...
	public static void updateTable_byName_cheque(JTable ui_table, String db_table, String chequeNo) {
		...
		Statement stmt = null;
//TODO add dealer name to invoice and cheque tables
		String sql = "SELECT `ChequeNo`, `InvoiceNo`, `DealerName`, `ChequeAmount`, `ChequeDate`, `ChequeExpireDate`, `ChequeExpireStatus`, `ChequePassStatus` FROM `cheque` WHERE `ChequeNo` LIKE  '"
				+ chequeNo + "%'";
		try {
			stmt = connection.prepareStatement(sql);
			ResultSet rs = stmt.executeQuery(sql);
